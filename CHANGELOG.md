...
- **Reloading the session list no longer spends a usage request** - `updateQuotaStatus` was called from 18 places, most of them session actions (Refresh Sessions, Reindex, Pin, Unpin, Rename, Archive, Unarchive, Ungroup, sort/scope toggles, the file watcher, the auto-refresh timer) that have nothing to do with the Claude usage endpoint. Only the three callers where a fresh reading is actually wanted (the quota settings changing, the activation reading, and the quota poll timer itself) now request live data; every other caller repaints the status bar from cache or the local estimate. The quota poll timer also now follows `claudeHistory.quota.claudeUsagePollSeconds` instead of a hardcoded 5 minutes, and re-arms itself when the setting changes.
